...
In this project, built a conversational recommendation system for fitness guidance using Retrieval-Augmented Generation (`RAG`), integrating `OpenAI LLMs` with a custom in-memory search engine. Applied principles relevant to `ad ranking and recommender systems` by optimizing query relevance, ranking exercise suggestions based on contextual intent (e.g., user goals, equipment), and evaluating response quality using `LLM-as-a-Judge`. Designed for scalability and traceability with `Grafana monitoring`, enabling real-time insight into usage, relevance metrics, and system costs—paralleling brand ads performance tracking.
...
```bash
psql -h localhost -U your_username -d course_assistant -W
```
To install psql on mac
